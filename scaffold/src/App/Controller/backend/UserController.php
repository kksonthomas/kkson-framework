...
class UserController extends BackendCRUDControllerBase
{
    /**
    ...
    public function main($crud)
    {
        ...
        $roleFieldType = new Dropdown(User::getBackendRoleList());
        ...
        $crud->rowAction(function ($user, $commonButtonClass) {
            $html = "";
            /** @var User $user */
            if ($user->role != User::ROLE_GENERAL_USER) {
                if (!Auth::isLoginAs() && Auth::isPermitted(Permission::PERMISSION_BACKEND_LOGIN_AS) && !$user->isSystemAdmin() && $user->active) {
                    $html .= "<a href='" . UrlUtils::burl("login_as/{$user->id}") . "' class='btn btn-success {$commonButtonClass}' style='margin-left:5px;'>登入</a>";
                } else {
                    $html .= "<a href='#' class='btn btn-success {$commonButtonClass} disabled' style='margin-left:5px;'>登入</a>";
                }
            }
            return $html;
        });
    ...
    {
        $crud->field("username")->setRequired(true);
